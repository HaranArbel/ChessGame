...
// this struct defines a board square
typedef struct Square{
	int row;
	int col;
}Square;

//this struct defines a move
typedef struct Move{
	Square from;
	Square to;
	unsigned int moved_piece;
	unsigned int taken_piece;
}Move;

// this struct defines an ArrayList that
// contains pointers to moves
...
/**
 * Returns a joined list that consists of the elements
 * of the first and the secons list
 * @param lst1 - the first list
 * @param lst2 - the second list
 * @return - a list with the elements of both lst1 and lst2
 */
